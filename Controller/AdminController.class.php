...
class AdminController extends CommonController{
			...
			{   
				...
				//查找管理员有没有此人
     			$srt = M("uclient")->where(array("admin_id"=>$id,'_logic'=>'or',"finance_admin_id"=>$id,"rank_admin_id"=>$id))->find();
	     		 ...
	     		 $carloan=M("carloan")->where("salesman=".$id)->find();
	...
	// public function sqrank(){

	// 	if(IS_POST){
	// 		$arr=I('post.sp_level');
	// 		$sp_level=implode(',',$arr);
	// 		$data['sp_level']=$sp_level;
	// 		$num=M("admin")->where('id='.I('post.salesmans'))->save($data);
	// 		if($num){
	// 			$this->error('权限添加成功...',U('Admin/Admin/sqrank'));
	// 		}else{
	// 			$this->error('权限添加失败...', U('Admin/Admin/sqrank'));
	// 		}
	// 	}

	// 	if($_GET['ywy']){
	// 		$sp_level=M("admin")->where('id='.I('get.ywy'))->getField('sp_level');
	// 		$s_level=explode(',', $sp_level);
	// 		$this->ajaxReturn($s_level);
	// 	}
	...
	public function sqrank(){
		...
		if(IS_POST){
			...
			foreach ($_POST as $val) {
				...
				array_shift($val);
				$level=implode($val,',');
				$num=M("admin")->where("id={$id}")->save(['sp_level'=>$level]);
				if($num){
					$row=$num;
				}
			}
		...
		if($_GET['branch']){
			$sp_level=M("admin")->where('level_id='.I('get.branch'))->Field("id,sp_level")->select();
			// var_dump($sp_level);
			$this->ajaxReturn($sp_level);
		}
