...
class MonetaryPrivateController extends CommonController{
	...
	{
	   parent::_initialize();
	   $this->isUclient = "MonetaryPrivate";
	}
	...
	public function index(){
		...
		$where[] = $kun;
		// var_dump($where);exit();
		$data=M("Cash")->where($where)->order("audit_status desc")->select();

		//当月到期还款的时间
		foreach ($data as $k => $v) {
			$time=date('Y-m');
			// $where=array("uid"=>$data[$k]['id'],'endtime'=>array('like',"%$time%"));
			$where=array("uid"=>$data[$k]['id'],'repayment'=>1);
			$arr=M("Cashdetails")->where($where)->order("num asc")->Field('id,endtime')->find();
			$data[$k]['etime']=$arr['endtime'];
		}
		...
		foreach ($data as $key => $val) {
			...
			$arr=M("Cashdetails")->where("uid={$data[$key]['id']} and repayment=1")->order('id asc')->Field('id,endtime,Principal')->select();
			...
			//应还利息跟未还利息
			$time=date('Y-m');
			$where=array("uid"=>$data[$key]['id'],'endtime'=>array('like',"%$time%"));

			$arrs=M("Cashdetails")->where($where)->Field('id,endtime,returninterest,nowinterest,Principal')->find();
			// var_dump($arrs[1]);
			$data[$key]['whlx']=$arrs['nowinterest'];
			$data[$key]['yhlx']=$arrs['returninterest'];
			// var_dump($arrs[1]);
		...
		//累计利息
			$data[$key]['cumulativeInterest']=M("cashdetails")->where("uid={$data[$key]['id']}")->getField("sum(nowinterest) as nowinterest");
		}
		...
		if($_GET['column']){
	    	...
	    	if($_GET['sort']==1){
	      	...
	      	if($_GET['f']==1){
	      		$flag=2;
	      		$position="style='background-position: -10px 0px;'";
	      	}else{
	      		$flag=1;
	      		$position="style='background-position: 10px 0px;'";
	      	}
		...
		foreach ($data as $k => $v) {
			...
			$tfee+=$v['fee'];
			$tmargin+=$v['margin'];
			$tcumulativeInterest+=$v['cumulativeInterest'];
			$tlatefee+=$v['latefee'];
		}
		...
		$this->assign("tcumulativeInterest",$tcumulativeInterest);
	...
	public function indexsd(){
		...
		$post = I('get.');
		$stime = strtotime($post['stime']." 00:00:00");
		$endtime = strtotime($post['endtime']." 23:59:59");
		if(IS_GET){
		...
		foreach ($data as $k => $v) {
			$time=date('Y-m');
			// $where=array("uid"=>$data[$k]['id'],'starttime'=>array('like',"%$time%"));
			$where=array("uid"=>$data[$k]['id'],'repayment'=>1);
			$arr=M("Cashdetails")->where($where)->Field('id,starttime')->find();
			$data[$k]['stime']=$arr['starttime'];
		}
		...
		foreach ($data as $key => $val) {
			...
			//滞纳金
			$latefee=M("Cash")->Field('money,returnlaf')->find($data[$key]['id']);
			$arr=M("Cashdetails")->where("uid={$data[$key]['id']} and repayment=1")->order('id asc')->Field('id,starttime,Principal')->select();
			...
			//应还利息跟未还利息
			$time=date('Y-m');
			$where=array("uid"=>$data[$key]['id'],'starttime'=>array('like',"%$time%"));

			$arrs=M("Cashdetails")->where($where)->Field('id,starttime,returninterest,nowinterest,Principal')->find();
			// var_dump($arrs[1]);
			$data[$key]['whlx']=$arrs['nowinterest'];
			$data[$key]['yhlx']=$arrs['returninterest'];
	...
	public function edit(){
		...
		//还款详情
		$infos=M("Cashinfo")->where("uid={$id}")->order("time desc")->select();
		$i=0;
		...
		$latefee=M("Cash")->Field('money,returnlaf,name,aid,dkbh,margin,channelPersonnel')->find($id);
		...
		$this->assign("margin",$latefee['margin']);
		$this->assign("cp",$latefee['channelpersonnel']);
		$this->display();
	}
	...
	public function listCustomer(){
		...
		if($_GET['t']){
			$this->display('MonetaryPrivate/listCustomers');
		}else{
			$this->display();
		}
	...
	public function delsystem(){
		...
		if($num){
				$this->success('删除成功',U('MonetaryPrivate/listqysystem'));
			}else{
				$this->error('删除失败',U('MonetaryPrivate/listqysystem'));
			}
	}
	...
	public function addindex(){
		...
		if($id){
			...
			M("applys")->where("id={$_GET['aid']}")->save(array("state_id"=>24));
			if ($_GET['leixing']=='sd'){
				$number=$_GET["num"]+1;
			}else{
				$number=$_GET["num"];
		...
		$endtime=implode($arr, "-");
				...
				//速贷速分贷月本金根止息时间
				if($_GET['leixing']=='sd' && ($i==($_GET["num"]+1))){
				...
				//判断是否是按天数，获取自定义结束时间
				if((($_GET["endtime"]-$_GET["starttime"])/86400)<30){
					$endtime=date('Y-m-d',$_GET["endtime"]);
					// $data["endtime"]= $endtime;
				}
			...
			if($Cashdetailsid>0){
				...
				if($_GET['leixing']=='sd'){
					$this->error("添加成功",U("MonetaryPrivate/indexsd"));
					// $this->redirect("MonetaryPrivate/indexsd");
				}else{
					// $this->redirect("MonetaryPrivate/index");
					$this->error("添加成功",U("MonetaryPrivate/index"));
				}
			}
	...
	public function updsd(){
		$arr=M("cashdetails")->field("id,nowinterest,returnpri,returninterest,starttime,arrears,principal")->where("uid={$_POST['id']} and repayment=1")->select();
		$infos=array();
		$infos["uid"]=$_POST['id'];
		$repayment_time=$_POST['repayment_time'];
		$infos["repaymentime"]=$repayment_time;
		$type=$_POST['type'];
		$infos["time"]=time();
		$infos["returnlf"]=0;
		$infos["explain"]='归还利息';
		// 逾期天数
		if(strtotime($repayment_time)>strtotime($arr[0]['starttime'])){
			$day=(strtotime($repayment_time)-strtotime($arr[0]['starttime']))/86400;
		}else{
			$day=0;
		}
		...
		if(count($arr)>1){
			if($ylx==$hkje){
				...
				$data["repayment_time"]=$repayment_time;
				$data["yqday"]=$day;
				if($hkje==$arrears){
					$data['repayment']=2;
					$data['Arrears']=0;
			...
			//判断是否归还利息
			if($arr[0]['returninterest']!=$arr[0]['nowinterest']){
					//判断金额除不尽，有小数
				if(($ylx-$hkje)<1){
						$data["repayment_time"]=$repayment_time;
						$data["returninterest"]=$ylx;
	...
	public function dels(){
		...
		if($num){
			...
			if($nums){
					if($type==1){
						$this->success('删除成功',U('MonetaryPrivate/index'));
					}else{
						$this->success('删除成功',U('MonetaryPrivate/indexsd'));
					}
				}else{
					if($type==1){
						$this->success('删除失败',U('MonetaryPrivate/index'));
					}else{
						$this->success('删除失败',U('MonetaryPrivate/indexsd'));
					}
			}
		}
	}
	...
	public function del(){
		...
		if($num){
				...
				$this->success('删除成功',U('MonetaryPrivate/listCustomer'));
			}else{
				$this->error('删除失败',U('MonetaryPrivate/listCustomer'));
			}
	}
	...
	public function infos(){
		...
		$this->assign('data',$data);
		if($_GET['t']){
			$this->display("MonetaryPrivate/infoss");
		}else{
			$this->display();
		}
	}
	...
	public function modify(){
		...
		// var_dump($datas);
		if($t=='sd'){
			$this->display("MonetaryPrivate/modifysd");
		}else{
			$this->display();
		}
	}
	...
	public function sdupd(){
		...
		$endtime=implode($arr, "-");
			...
			if($Cashdetailsid>0){
				if($_GET['leixing']=='sd'){
					$this->success('更新成功',U('MonetaryPrivate/indexsd'));
					// $this->redirect("MonetaryPrivate/indexsd");
				}else{
					// $this->redirect("MonetaryPrivate/index");
					$this->success('更新成功',U('MonetaryPrivate/index'));
				}
			}

		}else{
			if($t=='sd'){
				$this->error('更新失败',U('MonetaryPrivate/indexsd'));
			}else{
				$this->error('更新失败',U('MonetaryPrivate/index'));
			}
	...
	public function addsd(){
		...
		$this->assign('data',$data);
		$this->display('MonetaryPrivate/addindexsd');
	}
	...
	public function addsfd(){
		...
		$data['money']=$data['money'];
		$this->assign('data',$data);
		$this->display('MonetaryPrivate/addindex');
	}
	...
	public function before_money(){
		...
		$csbj=I('post.csbj');
		$ids=I('post.ids');
		$repayment_time=I('post.repayment_time');
		//归还本金是否是最小一期
		$nums=M("cashdetails")->where(array('repayment'=>1,'uid'=>$ids))->Field("min(num) as num,starttime")->find();
		// 逾期天数
		if(strtotime($repayment_time)>strtotime($nums['starttime'])){
			$day=(strtotime($repayment_time)-strtotime($nums['starttime']))/86400;
		}else{
			$day=0;
		}
		// echo $hkje;exit();
		if($no==$nums['num']){
			...
			M("cashdetails")->where(array('num'=>$no,'uid'=>$ids))->save(array("returnpri"=>($hkje+$ghbj['returnpri']),'yqday'=>$day,'repayment_time'=>$repayment_time));
			...
			//排除最后一期(还本金重新计算利息)
			$detailsIds=$cids;
			unset($detailsIds[array_search(max($detailsIds), $detailsIds)]);
			$where[]=array("id"=>array('in',$detailsIds));
	...
	public function upd_fj(){
		...
		if($row){
			M("cashinfo")->add(array("uid"=>$ids,'time'=>time(),'sum'=>$hkje,'returnlf'=>0,'explain'=>'归还罚息金额','repaymentime'=>$repayment_time));
			echo "还款成功";
		}else{
			echo "还款失败";
		}
	...
	public function newsd(){
		...
		if($flag){
			...
			if($row){
				//修改cash期数
				M("cash")->where("id={$id}")->save(array("num"=>($endqs['num']-1),"endtime"=>strtotime($et."00:00:00")));
				//新增一期
				...
				$array['nowinterest']=$data[count($data)-2]['nowinterest'];
				...
				if($num){
					$this->success("成功加入一期", "/Admin/MonetaryPrivate/editsd/id/{$id}");
				}else{
					$this->error("新增失败", "/Admin/MonetaryPrivate/editsd/id/{$id}");
				}
			...
			$this->error("贷款已还完，不能再新增", "/Admin/MonetaryPrivate/editsd/id/{$id}");
		}
	...
	public function setState(){
		...
		$id=$_GET['id'];
		if($v>0 && $id>0){
			$num=M("cash")->where("id={$id}")->save(array('audit_status'=>$v));
			if($num){
				echo "<script>alert('已通过审核');window.history.go(-1);location.reload()</script>";
			}else{
				echo "<script>alert('审核失败');window.history.go(-1);location.reload()</script>";
			}
		}
	}

	//分离本金，等之前的数据还完可删掉
	public function flbj(){
		$id=$_GET['id'];
		$data=M("cashdetails")->where("uid={$id}")->order("num asc")->select();
		$flag=false;
		foreach ($data as $key => $value) {
