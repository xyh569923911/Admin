...
class ApplyPrivateController extends CommonController{
	...
	public function add(){
			...
			if(!$info) {  
				...
				if($ids>0){
					...
					$this->success('添加资料成功...','http://47.92.119.237/Admin/MonetaryPrivate/ipadSystem');
				}else{
					// echo "<script>alert('添加资料失败')</script>";
					$this->error('添加资料失败...', U('ApplyPrivate/index'));
				}
			} 
	}
	...
	public function upd(){
		...
		if($num>0 || $nums>0){
			if($_POST['t']){
					$this->success('修改成功...', U('/Admin/MonetaryPrivate/listCustomer/t/1'));
				}else{
					$this->success('修改成功...', U('/Admin/MonetaryPrivate/listCustomer'));
				}
		}else{
			if($_POST['t']){
					$this->error('修改失败...', U('/Admin/MonetaryPrivate/listCustomer/t/1'));
				}else{
					$this->error('修改失败...', U('/Admin/MonetaryPrivate/listCustomer'));
				}
		}
	}
	...
	public function yewuone(){
		...
		if($_FILES){
			...
			if($_FILES["{$_POST['zd']}"]['name']){
				...
				if($row){
					if($_POST['t']){
							echo "<script>alert('资料保存成功');window.location.href='/Admin/ApplyPrivate/yewuone/t/1?id={$id}'</script>";
						}else{
							echo "<script>alert('资料保存成功');window.location.href='/Admin/ApplyPrivate/yewuone?id={$id}'</script>";
						}
				}else{
					if($_POST['t']){
							echo "<script>alert('资料保存失败');window.location.href='/Admin/ApplyPrivate/yewuone/t/1?id={$id}'</script>";
						}else{
							echo "<script>alert('资料保存失败');window.location.href='/Admin/ApplyPrivate/yewuone?id={$id}'</script>";
						}
				}
			}else{
				$this->error("请上传资料");
		...
		if($_GET['t']){
			$this->display("ApplyPrivate/yewuones");
		}else{
			$this->display();
		}
	}
	...
	public function over(){
		...
		if($num){
			if($_GET['t']){
				$this->success("已完成。。。", U('/Admin/MonetaryPrivate/listCustomer/t/1'));
			}else{
				$this->success("已完成。。。", U('/Admin/MonetaryPrivate/listCustomer'));
			}
		}else{
			if($_GET['t']){
				$this->success("未完成。。。", U('/Admin/MonetaryPrivate/listCustomer/t/1'));
			}else{
				$this->success("未完成。。。", U('/Admin/MonetaryPrivate/listCustomer'));
			}
		}
	}
	...
	public function yewutwo(){
		...
		$this->assign('id',$_GET['id']);
		if($_GET['t']){
			$this->display("ApplyPrivate/yewutwos");
		}else{
			$this->display();
		}
	...
	public function qr_info(){
		...
		if($row){
			if($_POST['t']){
				$this->error("资料保存成功","/Admin/ApplyPrivate/yewutwo/t/1?id={$cid}");
			}else{
				$this->error("资料保存成功","/Admin/ApplyPrivate/yewutwo?id={$cid}");
			}
			
		}else{
			if($_POST['t']){
				$this->error("资料保存失败","/Admin/ApplyPrivate/yewutwo/t/1?id={$cid}");
			}else{
				$this->error("资料保存失败","/Admin/ApplyPrivate/yewutwo?id={$cid}");
			}
		}
	}
	...
	public function yewuthree(){
		...
		$this->assign('id',$_GET['id']);
		if($_GET['t']){
			$this->display("ApplyPrivate/yewuthrees");
		}else{
			$this->display();
		}
	}
	...
	public function qrfy(){
		...
		if($row){
			$this->error("资料保存成功","/Admin/ApplyPrivate/yewuthree?id={$_POST['cid']}");	
		}else{
			$this->error("资料保存失败","/Admin/ApplyPrivate/yewuthree?id={$_POST['cid']}");
	...
	public function investigate(){
		if(IS_POST){
			...
			if($row){
				...
				$this->error("资料保存成功","/Admin/ApplyPrivate/investigate?id={$aid}");
					
			}else{
				$this->error("资料保存失败","/Admin/ApplyPrivate/investigate?id={$aid}");
			}
		}else{
		...
		$this->assign('id',$id);
		if($_GET['t']){
			$this->display("ApplyPrivate/investigates");
		}else{
			$this->display();
		}
	}
	...
	public function loan_approval(){
		if(IS_POST){
			...
			if($row){
				if($_POST['t']){
					$this->error("资料保存成功","/Admin/ApplyPrivate/loan_approval/t/1?id={$aid}");
				}else{
					$this->error("资料保存成功","/Admin/ApplyPrivate/loan_approval?id={$aid}");
				}
			}else{
				if($_POST['t']){
					$this->error("资料保存失败","/Admin/ApplyPrivate/loan_approval/t/1?id={$aid}");
				}else{
					$this->error("资料保存失败","/Admin/ApplyPrivate/loan_approval?id={$aid}");
				}
			}
		}else{
			$data=M("cashfiles")->where("aid={$_GET['id']}")->find();
		...
		$this->assign('id',$_GET['id']);
		if($_GET['t']){
			$this->display("ApplyPrivate/loan_approvals");
		}else{
			$this->display();
		}
	}
	...
	public function sign(){
		if(IS_POST){
			...
			if($row){
				if($_POST['t']){
					$this->error("资料保存成功","/Admin/ApplyPrivate/sign/t/1?id={$aid}");
				}else{
					$this->error("资料保存成功","/Admin/ApplyPrivate/sign?id={$aid}");
				}
			}else{
				if($_POST['t']){
					$this->error("资料保存失败","/Admin/ApplyPrivate/sign/t/1?id={$aid}");
				}else{
					$this->error("资料保存失败","/Admin/ApplyPrivate/sign?id={$aid}");
				}
			}
		}else{
		...
		$this->assign('id',$_GET['id']);
		if($_GET['t']){
			$this->display("ApplyPrivate/signs");
		}else{
			$this->display();
		}
	}
